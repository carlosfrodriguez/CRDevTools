...
local $Kernel::OM = Kernel::System::ObjectManager->new(
    LogObject => {
        LogPrefix => 'OTRS-cr.DevTypeDelete.pl',
    },
);
...
$CommonObject{DevTypeObject} = Kernel::System::CR::Dev::Type->new(%CommonObject);
...
if ( $Opts{h} ) {
    ...
    if ( $Opts{i} ) {
        ...
        if ( $Opts{i} !~ m{\A\d+\z} ) {
            print "The TypeID $Opts{i} is invalid!\n";
            _Help();
            exit 0;
        }
        _Delete(
        ...
        # check if ID is numeric valid
        if ( $Opts{r} !~ m{\A(\d+)\.\.(\d+)\z} ) {
            print "The TypeID $Opts{r} is invalid!\n";
            _Help();
            exit 0;
        }
        my @ItemIDs = ( $1 .. $2 );
...
sub _List {
    ...
    # search all tickets
    my %List = $CommonObject{TypeObject}->TypeList(
        Valid => 0,
    );
...
sub _Search {
    ...
    # search all users
    my %List = $CommonObject{DevTypeObject}->TypeSearch(
        %SearchOptions,
        Valid => 0,
    );
...
sub _Output {
    ...
    for my $ItemID (@ItemIDs) {
        ...
        # get item details
        my %Item = $CommonObject{TypeObject}->TypeGet(
            ID => $ItemID,
        );
        next ITEM if !%Item;
...
sub _Delete {
    ...
    for my $ItemID (@ItemsToDelete) {
        ...
        if ( !%Item ) {
            print "The type with ID $ItemID does not exist!\n";
            $Failed = 1;
            next ITEMID;
        }
        ...
        if ( $TicketIDs[0] ) {
            if ( $Param{DeleteTickets} ) {
                for my $TicketID (@TicketIDs) {
                    ...
                    if ($Success) {
                        print "Ticket $TicketID deleted as it was used by type $ItemID\n";
                    }
                    else {
                ...
                print "Could not delete type $ItemID due the following tickets use it:\n";
                for my $TicketID (@TicketIDs) {
        ...
        # delete ticket
        my $Success = $CommonObject{DevTypeObject}->TypeDelete(
            TypeID => $ItemID,
            UserID => 1,
        );

        if ( !$Success ) {
            print "Can't delete type $ItemID\n";
            $Failed = 1;
        }
    }
    return $Failed;
...
sub _Help {
    print <<'EOF';
cr.DevTypeDelete.pl - Command line interface to delete ticket types.

Usage: cr.DevTypeDelete.pl
Options:
    -a list                           # list all types

    -a search -n *some name*          # search types with specified name (wild cards are allowed)

    -a delete -i 123                  # deletes the type with ID 123
