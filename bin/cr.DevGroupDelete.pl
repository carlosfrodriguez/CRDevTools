...
if ( $Opts{h} ) {
    ...
    if ( $Opts{i} ) {
        ...
        if ( $Opts{i} !~ m{\A\d+\z} ) {
            print "The GroupID $Opts{i} is invalid!\n";
            _Help();
            exit 0;
        }
        _Delete(
        ...
        # check if ID is numeric valid
        if ( $Opts{r} !~ m{\A(\d+)\.\.(\d+)\z} ) {
            print "The GroupID $Opts{r} is invalid!\n";
            _Help();
            exit 0;
        }
        my @ItemIDs = ( $1 .. $2 );
...
sub _Delete {
    ...
    for my $ItemID (@ItemsToDelete) {
        ...
        if ( !%Item ) {
            print "The Group with ID $ItemID does not exist!\n";
            $Failed = 1;
            next ITEMID;
        }
...
sub _Help {
    print <<'EOF';
cr.DevGroupDelete.pl - Command line interface to delete ticket Priorities.

Usage: cr.DevGroupDelete.pl
Options:
    -a list                           # list all Groups

    -a search -n *some name*          # search Groups with specified name (wild cards are allowed)

    -a delete -i 123                  # deletes the Group with ID 123
    -a delete -r 5..10                # deletes the Groups with IDs between 5 and 10
